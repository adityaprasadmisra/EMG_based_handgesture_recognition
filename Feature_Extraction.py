...
def extract_features(window):
    """
    Takes a window (50 rows × 2 channels)
    Returns 10 features per channel:
      MAV, RMS, Variance, Std Dev, Zero Crossing Count
    Total: 2 channels × 5 features = 10 features
    """
    features = []
    for ch in ['emg1', 'emg2']:
        col = window[ch].values.astype(float)

        # Mean Absolute Value — most important EMG feature
        ...
        # Zero Crossing Count — frequency content indicator
        zc = np.sum(np.diff(np.sign(col - np.mean(col))) != 0)

        features.extend([mav, rms, var, std, zc])

    return features  # 2 channels × 5 features = 10 values
